...
exports.parseBestServerColorString = parseBestServerColorString;
exports.parseStudyXml = parseStudyXml;
exports.parseBestServerDictionary = parseBestServerDictionary;
exports.parseBestServerColors = parseBestServerColors;
 ...
 */
function parseBestServerColorString(colorStr) {
    if (!colorStr)
        return emptyRange();
    ...
    for (const part of parts) {
        ...
        if (colonIdx === -1) {
            ...
            if (c.length === 4)
                noDataColor = { r: c[1], g: c[2], b: c[3], a: c[0] };
        }
        else {
...
async function parseStudyXml(xmlFilePath) {
    ...
    if (!link)
        throw new Error('Estructura XML no reconocida');
    const studyName = link['Name']?.[0] || 'Estudio';
    const serviceName = link['Service']?.[0]?.['Link']?.[0]?.['ServiceName']?.[0] || '5G';
    // Área geográfica
     ...
     */
    function findFieldRecursive(obj, name) {
        ...
        if (v) {
            if (typeof v === 'string' && v.trim())
                return v.trim();
            if (typeof v === 'object' && typeof v._ === 'string' && v._.trim())
                return v._.trim();
        }
    ...
    for (const cov of coverages) {
        ...
        if (!covLink)
            continue;
        const name = field(covLink, 'Name');
        const id = field(covLink, 'Id') || name;
        const layerRaw = field(covLink, 'Layer');
 ...
 */
async function parseBestServerDictionary(signalDir) {
    const dict = new Map();
    if (!fs.existsSync(signalDir))
        return dict;
    // Buscar mtxbsres.xml recursivamente
    const allFiles = [];
    function collectMtx(dir) {
        for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
            const full = path.join(dir, entry.name);
            if (entry.isDirectory())
                collectMtx(full);
            else if (entry.name.toLowerCase() === 'mtxbsres.xml')
                allFiles.push(full);
        }
    }
 ...
 */
async function parseBestServerColors(signalDir) {
    if (!fs.existsSync(signalDir))
        return emptyRange();
    const allFiles = [];
