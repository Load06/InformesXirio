...
function applyEntityCorrection(entity, corrections) {
    const key = entity.code ?? entity.name ?? '';
    const correctedTotal = corrections[key];
    if (!correctedTotal || correctedTotal <= 0)
        return entity;
    return {
        ...entity,
 ...
 */
function oPara(text, style, bold = false, keepNext = false) {
 ...
 */
function oDrawing(rId, cxEmu, cyEmu, id, name) {
    const n = escXml(name);
    return `<w:drawing>
...
function tCell(w, content, align) {
...
function tCellBold(w, text, align) {
    const jc = align ? `<w:jc w:val="${align}"/>` : '';
    return `<w:tc>
    <w:tcPr>
...
function tCellText(w, text, align) {
...
function buildThresholdTable(title, thresholds, absVals, pctVals, showAbs, showPct, absLabel, pctLabel, totalWidth) {
    const thrW = 1200;
...
function buildBsLegendXml(colorRange) {
    if (!colorRange?.stops?.length)
        return oEmptyPara();
    const rows = colorRange.stops.map((s) => {
...
function buildStatsInlineXml(stats, _unit, opts, corrections = {}) {
    ...
    if (hasPopData && (opts.showPopAbs || opts.showPopPct)) {
        xml += buildThresholdTable('Cobertura de población', stats.thresholds, opts.showPopAbs ? entity.populationAbs : [], opts.showPopPct ? entity.populationPct : [], !!opts.showPopAbs, !!opts.showPopPct, 'Hab.', '%', STATS_INNER_TWP);
    }
    // Surface coverage
    const hasSurfData = entity.surfaceAbs.length > 0 || entity.surfacePct.length > 0;
    if (hasSurfData && (opts.showSurfAbs || opts.showSurfPct)) {
        xml += buildThresholdTable('Cobertura de superficie', stats.thresholds, opts.showSurfAbs ? entity.surfaceAbs : [], opts.showSurfPct ? entity.surfacePct : [], !!opts.showSurfAbs, !!opts.showSurfPct, 'km²', '%', STATS_INNER_TWP);
    }
    return xml || oEmptyPara();
}
/** Stats cell XML: decide which component to render. */
function buildStatsCellXml(section, colorRanges, opts, corrections = {}) {
...
function needsOverflowPage(section, opts) {
    return (opts.showStats &&
...
function buildOverflowXml(stats, opts, corrections = {}) {
    ...
    if (showPop && entities.some((e) => e.populationAbs.length > 0 || e.populationPct.length > 0)) {
    ...
    if (showSurf && entities.some((e) => e.surfaceAbs.length > 0 || e.surfacePct.length > 0)) {
 ...
 */
function extractDocOpenTag(docXml) {
    ...
    if (start === -1)
        return '<w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main" xmlns:wp="http://schemas.openxmlformats.org/drawingml/2006/wordprocessingDrawing" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships">';
 ...
 */
function addImageRels(relsXml, images) {
...
async function generateDocx(options) {
    ...
    const entitySurfaceCorrections = options.entitySurfaceCorrections ?? {};
    ...
    const selectedSet = new Set(options.selectedResults.map((r) => `${r.layer}-${r.type}`));
    ...
    layerOrder.forEach((lid, layerIdx) => {
        const layer = metadata.layers.find((l) => l.id === lid);
        if (!layer)
            return;
        const layerResults = metadata.availableResults.filter((r) => r.layer === lid && selectedSet.has(`${r.layer}-${r.type}`));
        layerResults.forEach((r, sectionIdx) => {
            resultSections.push({
                layerId: lid,
    ...
    const zip = new pizzip_1.default(plantillaBuf);
    ...
    for (const section of resultSections) {
        ...
        // Stats cell XML
        const statsXml = buildStatsCellXml({ ...section, colorRange: colorRanges[section.type] }, colorRanges, statsOptions, entitySurfaceCorrections);
        if (img) {
