...
import { AvailableResult, Layer, RESULT_LABELS } from '../types/study';
 ...
 */
export async function extractZipFromBuffer(buf: Buffer, destDir: string): Promise<void> {
  await fs.promises.mkdir(destDir, { recursive: true });
 ...
 */
export function detectSignalResults(signalDir: string, sessionDir: string): {
  layers: Layer[];
  ...
  if (!fs.existsSync(signalDir)) return { layers, results };
 ...
 */
export function detectInterferenceResults(interferenceDir: string, sessionDir: string): AvailableResult[] {
  const results: AvailableResult[] = [];

  if (!fs.existsSync(interferenceDir)) return results;
 ...
 */
export async function readInterferenceStats(
  interferenceDir: string
): Promise<Record<string, Array<{ threshold: string; percentage: number }>>> {
  const stats: Record<string, Array<{ threshold: string; percentage: number }>> = {};

  if (!fs.existsSync(interferenceDir)) return stats;
 ...
 */
export function enrichLayerNames(
  layers: Layer[],
  xmlLayers: Layer[]
): Layer[] {
  ...
  return layers.map((layer, idx) => {
    if (layer.isGlobal) return layer;
